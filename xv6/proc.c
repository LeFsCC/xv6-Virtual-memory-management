...
int growproc(int n)
{
  ...
  sz = curproc->sz;
  // if new stacksize is bigger than the rest avaliable memory size, cast fault
  if ((sz + n) > USERSTACKTOP - curproc->stacksize - PGSIZE)
  {
    return -1;
  }
...
int fork(void)
{
  ...
  *np->tf = *curproc->tf;
  ...
  if (mystrcmp(curproc->name, "init") != 0 && mystrcmp(curproc->name, "sh") != 0)
  {
    ...
    {
      if (vpwrite(np, buf, offset, nread) == -1)
        panic("[ERROR] Copying vpfile in fork().");
      offset += nread;
    }
  }
...
int make_shrmem(int sign)
{
  ...
  int i = 0, j = 0, ret = 0;
  struct proc *curproc = myproc();
  for (j = 0; j < PROC_SHR_MEM_NUM; j++)
    if (curproc->shrmem_sigs[j] == 0)
    {
      break;
    }
  if (j == PROC_SHR_MEM_NUM)
  {
    release(&shrmem_lock);
    return -1;
  }
...
int remove_shrmem(int sign)
{
  ...
  for (i = 0; i < SHR_MEM_NUM; i++)
  {
    if (shrmems[i].sig == sign)
    {
      kfree(shrmems[i].addr);
      shrmems[i].addr = (void *)-1;
      shrmems[i].sig = 0;
      break;
    }
  }
...
int read_shrmem(int sign, char *buffer)
{
  ...
  for (i = 0; i < SHR_MEM_NUM; i++)
  {
    if (shrmems[i].sig == sign)
    {
      memmove(buffer, shrmems[i].addr, PGSIZE);
      break;
    }
  }
  if (i == SHR_MEM_NUM)
...
int write_shrmem(int sign, char *buffer)
{
  ...
  for (i = 0; i < SHR_MEM_NUM; i++)
  {
    if (shrmems[i].sig == sign)
    {
      strncpy(shrmems[i].addr, buffer, len + 1);
      break;
    }
  }
  if (i == SHR_MEM_NUM)
